...
$sql = "SELECT rc_user.*,user_register.* FROM rc_user INNER JOIN user_register ON rc_user.user_id = user_register.user_id where user_register.user_id like '%$search_text%' or rc_user.rc_vehicle_no like '%$search_text%' ";
// echo $sql;
...
mysqli_query($con,"SET SESSION collation_connection ='utf8_unicode_ci'");
 ...
 if($result = $con->query($sql))
 {
   ...
   while($data = mysqli_fetch_assoc($result)) {   
                              ...
                              echo '     <tr class="well"  id="'.$data["user_id"].'">
                                     <td>'.$data["r_id"].'</td>
                                     <td>
                                         <div class="media">
