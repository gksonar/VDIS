...
<body>
  ...
  require 'db.php';
  extract($_POST);
 // $select = "SELECT user_licence.*,user_register.* FROM user_licence INNER JOIN user_register ON user_licence.user_id = user_register.user_id where user_register.user_id='$user_id'";
    $select = "SELECT user_licence.*,user_register.* FROM user_licence INNER JOIN user_register ON user_licence.user_id = user_register.user_id where user_register.user_id='$user_id'";
      ...
      while ($data = $result->fetch_assoc())
      {
          echo '<div class="container">
                                ...
                                <dl class="row mb-4">
                                    ...
                                    <dd class="col-7 text-right">'.$data["l_issue_date"].'</dd>
                                    <dt class="col-5  font-weight-normal text-left">User Email</dt>
                                    <dd class="col-7 text-right">'.$data["l_expiry_date"].'</dd>
